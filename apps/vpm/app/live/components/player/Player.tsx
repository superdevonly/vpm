"use client";
import * as Player from "@livepeer/react-latest/player";
...
import {
  ...
  PlayIcon,
  UnmuteIcon,
} from "@livepeer/react-latest/assets";
import { Settings } from "./Settings";

export function PlayerWithControls({ playbackId }: { playbackId: string }) {
  const [src, setSrc] = useState();
  useEffect(() => {
    const fetchPlayback = async () => {
      try {
        const res = await fetch(
          `/api/get-playbackinfo?playbackId=${playbackId}`
        );
  ...
  if (!src) {
    return (
      <PlayerLoading>
        <div className="flex flex-col inset-0 justify-center items-center">
  ...
  return (
    <div className="w-full mx-auto">
      <Player.Root
        ...
        src={src}
      >
        <Player.Container className="h-full w-full overflow-hidden rounded-md bg-gray-950 border data-[playing=true]:outline-white/80 data-[playing=true]:outline-2 data-[fullscreen=true]:outline-none data-[fullscreen=true]:rounded-none transition-all">
          <Player.Video
            title="Live stream"
          ...
          <Player.ErrorIndicator
            matcher="offline"
            className="w-full h-full absolute select-none animate-in fade-in-0 text-center bg-black/80 backdrop-blur-lg flex flex-col items-center justify-center gap-4 duration-1000 data-[visible=true]:animate-in data-[visible=false]:animate-out data-[visible=false]:fade-out-0 data-[visible=true]:fade-in-0"
          >
...
export const PlayerLoading = ({ children }: { children?: React.ReactNode }) => (
  <div className="relative w-full px-3 py-2 gap-3 flex-col-reverse flex aspect-video mx-auto animate-pulse bg-white/10 overflow-hidden rounded-sm">
